...
export abstract class TextAnimation {
    ...
    protected doAnimation(element: HTMLDivElement, duration: number = 1000, delay: number = 0, pos?: [ number, number ]): Promise<void> {
...
export class PreTextsAnimation extends TextAnimation {
    ...
    private async isRaul(): Promise<boolean> {
        return this.domainAndItselfIpMatches('raulwhite.duckdns.org');
    }

    private async isAndres(): Promise<boolean> {
        return this.domainAndItselfIpMatches('nopunyirperaqui.amgxv.tech');
    }
    ...
    private async isFrances(): Promise<boolean> {
        return Promise.resolve(window.navigator.languages.find(lang => lang.includes('fr')) !== undefined);
    }

    private async isCatala(): Promise<boolean> {
        return Promise.resolve(window.navigator.languages.find(lang => lang.includes('ca')) !== undefined);
    }
...
export class HappyNewYearAnimation extends TextAnimation {
    ...
    public async animate(): Promise<void> {
        const topPadding = 4;
        const leftPadding = 8;
        const linesHeights = this.textContainers
            .map(line => line.reduce((prev, char) => Math.max(prev, char.clientHeight), 0));
        const linesHeight = linesHeights
            .reduce((prev, lineHeight) => prev + lineHeight + topPadding, 0);
        const linesWidths = this.textContainers
            .map(line => line.reduce((prev, char) => prev + char.clientWidth + leftPadding, 0));
        ...
        for(let y = 0; y < this.textContainers.length; y++) {
            ...
            const h = ((height - linesHeight) / 2 + prevLines.reduce((prev, curr) => prev + curr + topPadding, 0));
            for(let x = 0; x < this.textContainers[y].length; x++) {
                const prevChars = this.textContainers[y].slice(0, x);
                const w = (width - linesWidths[y]) / 2 + prevChars.reduce((prev, curr) => prev + curr.clientWidth + leftPadding, 0);

                animationPromises.push(this.doAnimation(this.textContainers[y][x], 3000, anime.random(0, 100), [ w, h ]));
            }
        }
