...
import('./fireworks')
  .then(({ Fireworks: FireworksImpl }) => {
    Fireworks = FireworksImpl
  })
  .catch((reason) => console.error(reason))
import('./texts')
  .then(({ HappyNewYearAnimation: Impl1, PreTextsAnimation: Impl2 }) => {
...
anime({
  ...
  begin: (anim) => {
    ...
    new EventListener((<any>anim.animatables[0]).target.querySelector('button'), 'click').once(
      (e) => {
        anim.pause()
        start(e)
      },
