...
function createTrail(x: number, y: number): Trail {
  ...
  const trail: Trail = {
    x:
      x +
      (Math.random() < 0.5 ? -1 : 1) *
        anime.random(0.3 * window.innerWidth, 0.1 * window.innerWidth),
...
console.log(
  'Fireworks animation adapted from http://animejs.com/documentation/assets/js/fireworks.js',
)
export class Fireworks implements IFireworks {
    ...
    this.clearCanvas = anime({
      ...
      update: () => {
        this.ctx.clearRect(
          0,
          0,
          this.pixelRatio * canvasEl.width,
          this.pixelRatio * canvasEl.height,
        )
      },
      autoplay: false,
    })
  ...
  public throwFirework(x: number, y: number): FireworkEvents {
    ...
    for (let i = 0; i < this.numberOfParticules; i++) {
      const randomOffsetColor = Color.rgb(
        frandom(-0.2, 0.2),
        frandom(-0.2, 0.2),
        frandom(-0.2, 0.2),
      )
