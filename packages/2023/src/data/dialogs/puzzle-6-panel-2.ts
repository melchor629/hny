...
const puzzle6Panel2 = newDialog(
  ...
  .addEntry('second-number-ok', 'code-panel', (builder) =>
    builder
      ...
      .setChoices((b) =>
        b
          .addNext('7', '7', 'third-number-ok')
          .addNext('6', '6', 'third-number-wrong')
          .addNext('1', '1', 'third-number-wrong')
          .addNext('9', '9', 'third-number-wrong'),
      ),
  )
    ...
    builder.setText((b) =>
      b
        ...
        .condition(
          ([f, s, t, ft]) => f && s && t && ft,
          (bb) =>
            bb.text('CODE ACCEPTED').run(() => useDungeonState.getState().puzzle6CodeAccepted(1)),
        )
        .condition(
          ([f, s, t, ft]) => !f || !s || !t || !ft,
